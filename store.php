...
if($store_template == "")
{
	echo "<p><Error!</b> Store setup is incomplete. Please save template you want to use for your store from the members area. <a href='index.php?a=login'>Click here to login to the store admin area ..</a></p>";
	exit;
}
...
function getmenu($ppmem_id,$ppaction)
{
	...
	$storemenu=$storemenu."<a href=store.php?mem_id=$ppmem_id>Home</a><br>";
	$storemenu=$storemenu."<a href=store.php?mem_id=$ppmem_id&action=showContact>Contact</a><br>";
	$storemenu=$storemenu."<a href=store.php?mem_id=$ppmem_id&action=showAbout>About</a></center><hr>";
	...
	if ($res1=mysql_query($query1))
	{
		...
		{
			$storemenu=$storemenu."<a href='store.php?mem_id=$ppmem_id&action=showCat&catName=$storeCats1[1]'>$storeCats1[1]</a><br>";
		}
	}
			...
			$item_data= <<<HTM
					<tr>
						...
						<td align="left" valign="top">
							...
							<a href="store.php?mem_id=$ppmem_id&action=showItem&catName=$ppcatName&itemID=$storeItemsData[3]">Click here for details</a></p>
						</td>
					</tr>
HTM;
...
function getBuyButton($pppmem_id,$pppitemID,$pitemTitle,$pitemQty,$pitemPrice,$pitemType)
{
	...
	{
		$notify_url=$baseurl."/paypal.php?mem_id=".$pppmem_id;
	}
	if($res1=mysql_query($query1))
	{
		$storeInfoData=mysql_fetch_array($res1);
      ...
      <input type="hidden" name="return" value="$baseurl/store.php?action=thankYou&mem_id=$pppmem_id&item_id=$pppitemID">
